...
results/att_calibration/att_mean_X.XX/ and computes the mean recycling rate
for each att_mean value.  Recycling rate is defined as:
...
def _recycling_rate_from_consumers_csv(path: str) -> float:
    ...
    Returns Waste Recycle / total waste summed across all agents and timesteps.
    Returns NaN if the file is empty or missing required columns.
    """
...
def _collect_results(results_base: Path) -> pd.DataFrame:
    """
    Walk results_base/att_mean_X.XX/ trees and compute per-run recycling rates.
    ...
    for att_dir in sorted(results_base.glob(att_mean_dir_re)):
        ...
        # Results may be nested inside recycle_rate_XXpct/RTN_run_.../ subdirs
        consumer_csvs: list[str] = sorted(
            glob.glob(str(att_dir / "**" / "Results_agents_consumers_run_*.csv"),
                      recursive=True)
        )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        metavar="DIR",
        help="Base directory written by the calibration TORC run (default: results/att_calibration).",
    )
    parser.add_argument(
        "--target-rate",
        type=float,
    ...
    ax.set_title(
        ...
        "(cost = $25/module, initial rate = 20%, n = 100 replicates)"
    )
