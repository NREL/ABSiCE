"""
Generate RecyclingCostsbyYearAllLandfills.csv from RTN shipment data.

This script processes RTN recycling shipment data and merges it with USPVDB
to create a recycling costs file with PCA and case_id information.
"""
...
def validate_merge(merged_df, shipments_df):
    ...
    if len(merged_df) > len(shipments_df):
        ...
        duplicated_mask = merged_df.duplicated(
            subset=['Site', 'State', 'Year', 'Quarter', 'Recycler Name'], 
            keep=False
        )
...
def generate_recycling_costs(
    shipments_file: str,
    uspvdb_file: str,
    output_file: str
):
    """
    Generate recycling costs file from shipments and USPVDB data.
    ...
    required_shipments_cols = ['Site', 'State', 'Year', 'Quarter', 
                               'Recycler', 'TotalCost_$', 'Shipped_kg']
    ...
    shipments_df = shipments_df.rename(columns={'Recycler': 'Recycler Name'})
    ...
    # find sites in shipments that have multiple entries in USPVDB
    ...
    merged_df = shipments_df.merge(
        ...
        how='left',
        suffixes=('', '_uspvdb'),
        # validate='many_to_one'  # Each shipment should match at most one USPVDB entry
    )
    ...
    # Calculate Cost: (TotalCost_$ / Shipped_kg) * 1000 → $/ton
    # (was * 0.0077 to give $/W; now $/ton to match landfill CSV and model cost units)
    ...
    output_df = merged_df[[
        'Year', 'Quarter', 'PCA', 'Recycler Name', 'case_id', 'Site', 'State', 'Cost'
    ]]
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Generate RecyclingCostsbyYearAllLandfills.csv from RTN shipment data'
    )
    
    parser.add_argument(
        '--shipments-file',
        type=str,
        default='/Users/pghosh/SOLAR/shipments_recycle_alllandfills.csv',
        help='Path to shipments CSV file (default: /Users/pghosh/SOLAR/shipments_recycle_alllandfills.csv)'
    )
    ...
    parser.add_argument(
        '--output-file',
        type=str,
        default='RTN/RecyclingCostsbyYearAllLandfills.csv',
        help='Path to output CSV file (default: RTN/RecyclingCostsbyYearAllLandfills.csv)'
    )
